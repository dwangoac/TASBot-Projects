...
while (true) do
    ...
    if (movie.active() == true) then
        ...
        if (movie_loaded == false) then
            ...
            output_filename = string.sub(movie_filename, 0, string.len(movie_filename)-4) .. ".vsync.r08";
        ...
        if (movie.framecount() > 1) then
            -- We are capturing all frames in this script.
            if (lagged == false) then
                startup_lag = false;
            end

            if (startup_lag == false) then
                frame = frame + 1;
