...
start_dump = function(filename)
    fh, err = io.open(filename .. ".latsf.r16m", "wb");
    if not fh then
        error("Error opening output file: " .. err);
    end
    print(string.format('Dumping to %s.latsf.r16m', filename));
...
on_latch = function()
    if dumpfile then
        ...
        tframe = movie.get_frame(movie.find_frame(movie.currentframe()) + sframe)
        sframe = sframe + 1
        for i = 0, 7, 1 do
            out = 0
            for j = 0, 15, 1 do
