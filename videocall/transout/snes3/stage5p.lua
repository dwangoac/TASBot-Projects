...
stage5p = function(stream, loadbase)
	...
	--local speed = 130;
	local odd_frame = false; --true;
	--Load DPBase=4016, stack on right address and latch controllers.
	stream:lda_imm(0x4016);
	stream:tcd();
	local odd_add = odd_frame and 1 or 0;
	print(odd_add);
	print(2*speed+odd_add);
	stream:lda_imm(0x1000 + 8 * (2 * speed + odd_add) - 1);		-- WTF?
	stream:tcs();
	stream:ldx_imm(speed);
	stream:lda_imm(1);
	stream:sta_dp(0);
	stream:stz_dp(0);
	if odd_frame then poll_sequence_core(stream); end
